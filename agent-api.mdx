...
The Suppio Agent API lets your backend send a support message with optional image attachments to one Suppio agent and receive a generated response.
...
<Steps>
  ...
  <Step title="Call the endpoint from your backend">
    Send the key as a bearer token to the documented endpoint and include a JSON body with `message` and any image `attachments`.
  </Step>
