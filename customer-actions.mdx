...
app.post(
  ...
  async (req, res) => {
    const timestamp = req.header("X-Suppio-Timestamp") ?? "";
    const supplied = req.header("X-Suppio-Signature") ?? "";
    const ageSeconds = Math.abs(Date.now() / 1000 - Number(timestamp));

    if (!Number.isFinite(ageSeconds) || ageSeconds > 300) {
      res.status(401).json({ error: "Expired request" });
      return;
    }
    ...
    if (event.event === "customer_action.test") {
      res.json({ status: "succeeded", summary: "Connection verified." });
      return;
    }

    // Deduplicate event.action_id before making a change.
    await executeYourAction(event.action.name, event.action.arguments);
    res.json({
      status: "succeeded",
...
For success, return a `2xx` response with:
...
Suppio uses `summary` and optional `data` to write the final answer. Your response can be up to 64 KB, and `summary` can be up to 2,000 characters.
