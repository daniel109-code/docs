...
- validates the event envelope and matches the header and body action IDs
...
<Warning>
  The example plan-change handler deliberately returns `handler_not_configured`. Replace that handler with your application transaction and replace the sample Discord allowlist with resource-level authorization before enabling the action.
</Warning>
...
This script sends the same signed connection-test event shape that the dashboard uses:
...
const response = await fetch("http://127.0.0.1:8787/suppio/actions", {
  method: "POST",
  headers: {
    "Content-Type": "application/json",
    "X-Suppio-Action-Id": actionId,
...
if (
  ...
  throw new Error(`Connection test failed: ${response.status} ${JSON.stringify(result)}`);
}
...
The test should print `Connection verified.`
...
Do not deploy this SQLite example to a platform that runs several independent instances against separate disks. Use one shared transactional database for `action_id` claims and stored results in a horizontally scaled deployment.
