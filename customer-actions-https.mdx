...
Suppio calls your customer action executor through a public HTTPS URL. The same endpoint can receive customer action requests and ticket closing notifications. Your executor can run on a VPS, inside a private network connected through a tunnel, or on a managed application platform.
...
flowchart LR
    ...
    E -->|"JSON result or 2xx acknowledgment"| H
    H -->|"HTTPS response"| S
```
...
| Authentication | Always verify `X-Suppio-Timestamp` and `X-Suppio-Signature`. Match `X-Suppio-Action-Id` to `action_id` for customer actions or `X-Suppio-Event-Id` to `event_id` for ticket closing notifications. |
...
| Request size | Accept request bodies up to 64 KB. |
| Response size | Return no more than 64 KB. Keep `summary` at or below 2,000 characters. |
| Response format | Return the JSON result described in [Return a result](/customer-actions#return-a-result) for customer actions. Return any `2xx` response for `ticket.resolved`; Suppio ignores that response body. |

Before each outbound request, Suppio resolves the hostname and continues only when every returned address is public. A hostname that has no public answer or includes any private or reserved answer is rejected. Suppio uses a strictly public network path and never follows redirects.
...
<Card title="Start with the complete Node.js executor" icon="code" href="/customer-action-executor-example">
  Use the runnable secure template before publishing the endpoint. It includes every request, signature, schema, authorization, and idempotency check required by the lifecycle below.
</Card>
...
- the [complete Node.js executor example](/customer-action-executor-example), or an equivalent implementation in your stack
...
<Steps>
  ...
  <Step title="Store the signing secret outside the application">
    ...
    HOST=127.0.0.1
    PORT=8787
    DATA_DIR=/var/lib/customer-actions
    NODE_ENV=production
    ```

    `ALLOWED_DISCORD_USER_IDS` is only the example's temporary global allowlist. Replace it with resource-level authorization in your handler. Never commit this file, paste its contents into support messages, or include it in application logs. If your application needs other credentials, store them in the same protected secret system or your provider's secret manager.
  </Step>

  <Step title="Run the executor as a service">
    Place `package.json`, its lockfile, and `server.mjs` in `/opt/customer-actions`, then install production dependencies with `npm ci --omit=dev`. Create `/etc/systemd/system/customer-actions.service`:
    ...
    ExecStart=/usr/bin/node /opt/customer-actions/server.mjs
    Restart=on-failure
    RestartSec=5
    UMask=0077
    ...
    StateDirectory=customer-actions
    ...
    `StateDirectory=customer-actions` creates `/var/lib/customer-actions` and keeps that path writable even with `ProtectSystem=strict`. Store the durable idempotency database there. Update `ExecStart` if your entry file uses a different path. Then enable the service:
...
- the application has a persistent database or durable store for `action_id` and `event_id` deduplication
...
For `customer_action.execute` and `customer_action.test`:

1. Confirm that `X-Suppio-Action-Id` matches `action_id`.
2. Return success without mutating data when `event` is `customer_action.test`.
3. Reject action names that are not in an explicit handler allowlist.
4. Validate every argument again in your application.
5. Authorize `actor.user_id` for the targeted account or resource.
6. Atomically claim `action_id` in durable storage. Return the stored result if that ID already completed, and never repeat a result whose completion is uncertain.
7. Perform the action once.
8. Store and return a `succeeded` or `failed` JSON result within 15 seconds. Return both known outcomes with HTTP `2xx`.

For `ticket.resolved`:

1. Confirm that `X-Suppio-Event-Id` matches `event_id`.
2. Validate `source`, `actor`, `conversation`, and `agent`.
3. Atomically store `event_id` and the work your integration must perform. If the same ID arrives again, acknowledge it without creating duplicate work.
4. Return any `2xx` response after the event is durably accepted. Suppio ignores the body.

Ticket closing notifications use at-least-once delivery. Suppio retries network failures, timeouts, HTTP `408`, HTTP `429`, and `5xx` responses. Your `event_id` claim prevents those retries from applying the same close operation twice.
...
- Persist `action_id` and `event_id` values before applying side effects.
...
<AccordionGroup>
  ...
  <Accordion title="Test connection succeeds but ticket closing returns HTTP 400">
    **Test connection** sends `customer_action.test`, not `ticket.resolved`.
...
- known failures return HTTP `2xx` with structured failure results
- the executor responds in less than 15 seconds
- **Test connection** succeeds
- the signed `ticket.resolved` smoke test returns `2xx` when you use ticket closing notifications
