...
| Request size | Accept action request bodies up to 64 KB. |
...
Before each request, Suppio's production relay resolves both `A` and `AAAA` records and continues only when every returned address is public. A hostname that has no public answer or includes any private or reserved answer is rejected. Outbound executor requests use a strictly public network path, and the relay never follows redirects.
...
9. Reject action names that are not in an explicit handler allowlist.
10. Validate every argument again in your application.
11. Authorize `actor.user_id` for the targeted account or resource.
12. Atomically claim `action_id` in durable storage. Return the stored result if that ID already completed, and never repeat a result whose completion is uncertain.
13. Perform the action once.
14. Store and return a `succeeded` or `failed` JSON result within 15 seconds. Return both known outcomes with HTTP `2xx`.
